"""
OXware Backup Encryption — AES-256-GCM
──────────────────────────────────────
...
def get_default_passphrase() -> str:
    """OXWARE_BACKUP_PASSPHRASE env veya /etc/oxware/.backup_passphrase'ten al."""
    pp = os.environ.get("OXWARE_BACKUP_PASSPHRASE", "")
    if pp:
        return pp
    p = Path("/etc/oxware/.backup_passphrase")
    if p.exists():
        try:
...
def encrypt_file(src: str, dst: str, passphrase: str = None) -> dict:
    ...
    if not pp:
        raise ValueError("Passphrase yok — OXWARE_BACKUP_PASSPHRASE ayarla")
...
def decrypt_file(src: str, dst: str, passphrase: str = None) -> dict:
    ...
    with open(src_p, "rb") as f:
        ...
        if magic != _MAGIC:
            raise ValueError(f"Bu dosya OXware encrypted backup değil: {src}")
        ver   = f.read(1)
