"""
k8s_csi.py — Kubernetes CSI (Container Storage Interface) for OXware
OXware v2.5.10 Cloud/K8s

Features:
  - generate_csi_manifests() — DaemonSet + StorageClass YAML for OXware CSI driver
  - list_volumes() — list PersistentVolumes backed by OXware storage
  - create_volume_claim(name, size_gb, storage_class) — create PVC spec
  - get_csi_status() — CSI driver health / node registration status

Config persisted to /var/lib/oxware/k8s_csi.json
...
_CSI_FILE = Path("/var/lib/oxware/k8s_csi.json")
_lock     = threading.Lock()

_DRIVER_NAME   = "csi.oxware.io"
_DRIVER_IMAGE  = "ghcr.io/oxware/csi-driver:v2.5.10"
_NODE_IMAGE    = "ghcr.io/oxware/csi-node:v2.5.10"
      ...
      containers:
        - name: oxware-csi-driver
          ...
          env:
            ...
            - name: OXWARE_API_URL
              value: "http://oxware-api.oxware-system.svc.cluster.local:5000"
            - name: OXWARE_API_TOKEN
              valueFrom:
                secretKeyRef:
                  name: oxware-csi-secret
                  key: api-token
          volumeMounts:
            - name: plugin-dir
              mountPath: /csi
...
kind: Deployment
metadata:
  name: oxware-csi-controller
  namespace: kube-system
  labels:
    app: oxware-csi-controller
spec:
  replicas: 1
  selector:
    matchLabels:
  ...
  template:
    metadata:
      labels:
        app: oxware-csi-controller
    spec:
      serviceAccountName: oxware-csi-controller-sa
...
kind: ServiceAccount
metadata:
  name: oxware-csi-controller-sa
  namespace: kube-system
---
apiVersion: rbac.authorization.k8s.io/v1
kind: ClusterRole
metadata:
  name: oxware-csi-controller-role
rules:
  - apiGroups: [""]
...
metadata:
  name: oxware-csi-controller-binding
subjects:
  - kind: ServiceAccount
    name: oxware-csi-controller-sa
    namespace: kube-system
...
def create_volume_claim(name: str, size_gb: int, storage_class: str = "oxware-standard") -> dict:
...
def get_csi_status() -> dict:
    ...
    node_count   = 0
    ready_nodes  = 0
    rc2, out2, _ = _kubectl(["get", "daemonset", "oxware-csi-node", "-n", "kube-system", "-o", "json"])
