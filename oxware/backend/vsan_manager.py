"""
OXware vSAN Manager — Ceph-backed distributed storage.
Provides vSAN-style abstraction over an existing Ceph cluster.
Storage: /var/lib/oxware/vsan_config.json
"""
import json, subprocess, logging, threading
from datetime import datetime, timezone
from pathlib import Path

log = logging.getLogger("oxware.vsan")
_CONFIG_FILE = Path("/var/lib/oxware/vsan_config.json")
...
def save_config(enabled, mode="ceph", ceph_mon="", ceph_user="admin",
                ceph_pool="oxware", nfs_server="", nfs_path="/exports/vsan",
                replication=2):
    with _lock:
        cfg = _load()
        cfg.update({
