"""
OXware Network Advanced — VXLAN, IPv6, BFD, DDoS mitigation
─────────────────────────────────────────────────────────────
...
_VXLAN_STORE = Path("/var/lib/oxware/vxlans.json")
_DDOS_CFG    = Path("/var/lib/oxware/ddos_config.json")
    ...
    try:
        ...
        for key in ["net.ipv6.conf.all.disable_ipv6",
            ...
            subprocess.run(["sysctl", "-w", f"{key}={val}"],
                           capture_output=True, timeout=5)
        Path("/etc/sysctl.d/99-oxware-ipv6.conf").write_text(
...
def ddos_apply(config: dict) -> dict:
    ...
    if cfg.get("enabled"):
        rules = [
            # SYN flood
            ["iptables", "-N", "OXWARE_SYN_FLOOD"],
            ["iptables", "-A", "INPUT", "-p", "tcp", "--syn", "-j", "OXWARE_SYN_FLOOD"],
            ["iptables", "-A", "OXWARE_SYN_FLOOD", "-m", "limit",
             "--limit", f"{cfg['syn_per_sec']}/s", "--limit-burst", "200",
             "-j", "RETURN"],
            ["iptables", "-A", "OXWARE_SYN_FLOOD", "-j", "DROP"],
            # ICMP flood
...
def ddos_clear() -> dict:
    """OXware DDoS kurallarını temizle."""
    try:
        subprocess.run(["iptables", "-F", "OXWARE_SYN_FLOOD"],
                       capture_output=True, timeout=5)
        subprocess.run(["iptables", "-X", "OXWARE_SYN_FLOOD"],
                       capture_output=True, timeout=5)
        return {"ok": True}
    except Exception as e:
        return {"ok": False, "error": str(e)}
