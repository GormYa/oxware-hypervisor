"""
OXware Disk Encryption — Live VM disk LUKS
───────────────────────────────────────────
qemu-img + cryptsetup ile VM disklerini AES-XTS-512 ile şifrele.
Key file: /etc/oxware/disk_keys/<vm_id>.key (perm 0600, root-only)
...
_KEY_DIR  = Path("/etc/oxware/disk_keys")
_REG      = Path("/var/lib/oxware/disk_encryption.json")
