...
Features:
  - detect_vm_network_mode(vm_id) → NAT / BRIDGE / ISOLATED / UNKNOWN
  - get_network_info(vm_id) → full networking context (mode, gateway, dhcp range)
  - validate_static_ip(vm_id, ip) → can this IP be used statically?
  - set_static_ip_cloudinit(vm_id, ip, gateway, netmask, dns) → inject via cloud-init
  - list_routable_networks() → networks where static IPs work
  - get_bridge_setup_status() → is oxbr0 configured?
  - suggest_ip_fix(vm_id) → human-readable recommendation
...
_STATE_FILE = Path("/var/lib/oxware/network_modes.json")
...
def get_network_info(vm_id: str) -> dict:
    """Full networking context for a VM — mode, gateway, DHCP range, static IP guidance."""
    ...
    if mode == "NAT":
        guidance = (
            ...
            f"({net_info.get('dhcp_start','?')}–{net_info.get('dhcp_end','?')}) as gateway {gateway}. "
        ...
        guidance = "VM is in isolated network — no external connectivity."
    else:
        guidance = "Unknown network mode — check virsh net-dumpxml for this network."
...
def validate_static_ip(vm_id: str, ip: str) -> dict:
    ...
    if mode == "NAT":
        ...
        return {
            "valid":   True,
            "warning": "NAT mode — static IP works internally but not reachable from outside.",
            "mode":    "NAT",
        }
...
def suggest_ip_fix(vm_id: str) -> dict:
    ...
    if mode == "NAT":
        steps = [
            "OPTION A — Bridge mode (recommended for real upstream IP):",
            "  1. Run: sudo bash /opt/oxware/scripts/setup-bridge.sh",
            "  2. Edit VM network in OXware UI → change to 'oxbridge'",
            "  3. Inside VM: set IP on upstream subnet, gateway = upstream router",
            "",
            "OPTION B — Static IP within NAT (no external access):",
            ...
            "",
            "OPTION C — Port forwarding (access specific ports):",
            "  OXware UI → Network → IPAM → Assign IP → creates DNAT rule",
        ]
    elif mode == "BRIDGE":
        steps = [
            "VM is in BRIDGE mode — static IPs work natively.",
            "Inside VM:",
...
def get_bridge_setup_status() -> dict:
    ...
    return {
        ...
        "message": (
            ...
            "To enable bridge networking: sudo bash /opt/oxware/scripts/setup-bridge.sh"
        ),
        "setup_command": "sudo bash /opt/oxware/scripts/setup-bridge.sh",
    }
