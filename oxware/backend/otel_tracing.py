...
Features:
  ...
  - configure(otlp_endpoint, enabled) — persisted to /var/lib/oxware/otel_config.json
...
_CONFIG_FILE = Path("/var/lib/oxware/otel_config.json")
...
def export_otlp() -> dict:
    ...
    for tid, spans in traces.items():
        ...
        resource_spans.append({
            "resource": {
                "attributes": [
                    {"key": "service.name", "value": {"stringValue": "oxware"}},
                ]
            },
            "scopeSpans": [{"scope": {"name": "oxware.tracing"}, "spans": scope_spans}],
        })
