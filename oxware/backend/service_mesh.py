...
OXware v2.5.9 Network Advanced 2
...
Config persisted to /var/lib/oxware/service_mesh.json
...
_MESH_FILE = Path("/var/lib/oxware/service_mesh.json")
_lock      = threading.Lock()

_KUBECONFIG_PATHS = [
    Path("/etc/oxware/kubeconfig"),
...
def generate_sidecar_config(service_name: str) -> dict:
    ...
    yaml_str = f"""\
# OXware v2.5.9 — Generated Sidecar Config for '{service_name}'
...
metadata:
  ...
  labels:
    ...
    oxware.io/service: "{service_name}"
spec:
  ingress:
    - port:
        number: {port}
...
metadata:
  ...
  namespace: oxware
spec:
  host: {service_name}.oxware.svc.cluster.local
  trafficPolicy:
    tls:
      mode: ISTIO_MUTUAL
    connectionPool:
...
def get_mtls_status() -> dict:
    ...
    if mesh["istio"]["status"] == "available":
        r = _run_istioctl("x", "check-inject", "-n", "oxware", timeout=15)
        if r.get("ok"):
            return {
                "ok":      True,
                "mesh":    "istio",
