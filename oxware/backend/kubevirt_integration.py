"""
kubevirt_integration.py — KubeVirt Integration for OXware
OXware v2.5.10 Cloud/K8s

Features:
  - detect_kubevirt() — check kubectl get kubevirt in any namespace
  - import_from_kubevirt(vmi_name) — import a KubeVirt VMI as OXware VM
  - export_to_kubevirt(vm_id) — generate KubeVirt VirtualMachine YAML for an OXware VM
  - list_kubevirt_vms() — list KubeVirt VMIs via kubectl

Config persisted to /var/lib/oxware/kubevirt.json
...
_STATE_FILE = Path("/var/lib/oxware/kubevirt.json")
_lock       = threading.Lock()

_KUBEVIRT_NS     = "kubevirt"
_OXWARE_DATA_DIR = Path("/var/lib/oxware/kubevirt_exports")
...
def _get_oxware_vm(vm_id: str) -> Optional[dict]:
    """Get OXware VM info from virsh."""
    try:
        r = subprocess.run(
            ["virsh", "dominfo", vm_id],
...
def import_from_kubevirt(vmi_name: str, namespace: str = "default") -> dict:
    """Import a KubeVirt VirtualMachineInstance as an OXware VM record.

    This reads the VMI spec from Kubernetes and creates a corresponding
    OXware VM configuration record (actual VM import / disk migration
...
def export_to_kubevirt(vm_id: str) -> dict:
    """Generate a KubeVirt VirtualMachine YAML for an OXware VM.

    Returns:
        dict with keys: ok, vm_id, yaml, filename, message
    """
    info = _get_oxware_vm(vm_id)
    if not info:
...
metadata:
  name: {name}
  labels:
    oxware.io/source: "oxware"
    oxware.io/vm-id: "{vm_id}"
      ...
      spec:
        storage:
          resources:
            requests:
              storage: 20Gi
          storageClassName: oxware-standard
        source:
          blank: {{}}
"""

    filename = f"{name}_kubevirt_vm.yaml"
    out_path = _OXWARE_DATA_DIR / filename
    ...
    try:
        _OXWARE_DATA_DIR.mkdir(parents=True, exist_ok=True)
