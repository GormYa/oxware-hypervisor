"""OXware audit-log retention policy + JSONL rotation.

The audit log (`/var/log/oxware/audit_chain.jsonl`), runbook history
(`/var/lib/oxware/runbook_history.jsonl`), and bulk-VM audit
(`/var/lib/oxware/bulk_audit.jsonl`) all grow without bound until this
module trims them. Default policy retains 90 days of entries with a
hard size cap of 200 MB per file.

State: /var/lib/oxware/audit_retention.json (policy settings only)
"""
...
log = logging.getLogger("oxware.audit_retention")
_POLICY_PATH = Path("/var/lib/oxware/audit_retention.json")
...
DEFAULT_POLICY = {
    ...
    "max_file_mb": 200,
    "files": [
        "/var/log/oxware/audit_chain.jsonl",
        "/var/lib/oxware/runbook_history.jsonl",
        "/var/lib/oxware/bulk_audit.jsonl",
    ],
