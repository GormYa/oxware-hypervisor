"""
OXware Compliance Scanner — CIS / NIST / PCI-DSS / HIPAA / ISO27001
────────────────────────────────────────────────────────────────────
...
Persistent: /var/lib/oxware/compliance_results.json
"""
...
_RESULTS = Path("/var/lib/oxware/compliance_results.json")
...
CONTROLS = [
    ...
    {"id": "NIST-AU-9",  "framework": "NIST",  "title": "Audit log integrity (AU-9)",
     "fn": lambda: os.path.exists("/var/log/oxware/feature_audit.jsonl") or _check_package_installed("auditd"),
    ...
    {"id": "PCI-10.5",   "framework": "PCI-DSS", "title": "Audit logs immutable / protected",
     "fn": lambda: _check_file_perm("/var/log/auth.log", 0o640) and _check_file_perm("/var/log/oxware", 0o750),
     "severity": "high", "remediation": "chmod 640 /var/log/auth.log; chmod 750 /var/log/oxware"},

    # HIPAA
    {"id": "HIPAA-312a", "framework": "HIPAA",   "title": "Access control — unique user IDs",
     "fn": lambda: True,  # Always pass if OXware RBAC in use
     "severity": "info",  "remediation": "OXware RBAC enforces unique user IDs"},
    {"id": "HIPAA-312e", "framework": "HIPAA",   "title": "Transmission security — TLS/SSL enabled",
     "fn": lambda: os.path.exists("/etc/oxware/oxware.crt"),
    ...
    {"id": "ISO-A.5.15", "framework": "ISO27001","title": "Access control — RBAC enforced",
     "fn": lambda: True, "severity": "info", "remediation": "OXware uses role-based access"},
    {"id": "ISO-A.8.16", "framework": "ISO27001","title": "Monitoring activities — audit log present",
     "fn": lambda: os.path.exists("/var/log/oxware/audit.log") or os.path.exists("/var/log/oxware/feature_audit.jsonl"),
