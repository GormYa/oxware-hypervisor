"""
OXware Forensics — Memory dump + Packet capture per VM
─────────────────────────────────────────────────────
- virsh dump <vm> <file>  → full RAM dump (paused or live)
- tcpdump on VM's tap interface → pcap file
- All artifacts stored under /var/lib/oxware/forensics/<vm_id>/<ts>/
...
_BASE = Path("/var/lib/oxware/forensics")
...
def list_artifacts(vm_id: str = None) -> list:
    """List all forensics artifacts under /var/lib/oxware/forensics/"""
    out = []
