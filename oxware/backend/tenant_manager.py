"""
OXware Tenant Manager — Hard Multi-Tenancy Isolation
─────────────────────────────────────────────────────
Tenants, kotalar, user→tenant ve vm→tenant atamaları.

  - Persistent state: /var/lib/oxware/tenants.json (atomic write)
  - Thread-safe (RLock)
...
_FILE = Path("/var/lib/oxware/tenants.json")
...
def _tenant_storage_path(tenant_id: str) -> str:
    return f"/var/lib/oxware/tenants/{tenant_id}"
