"""
OXware App-Consistent Snapshot — v2.5.7
────────────────────────────────────────
...
Persistent state: /var/lib/oxware/consistent_snapshots.json
"""
...
_DATA_FILE  = Path("/var/lib/oxware/consistent_snapshots.json")
_HOOKS_FILE = Path("/var/lib/oxware/app_snapshot_hooks.json")
...
def create_consistent_snapshot(vm_id: str, name: str, freeze_fs: bool = True) -> dict:
    ...
    try:
        ...
        xml = (
            ...
            f'<description>OXware app-consistent {ts}</description>'
            f'</domainsnapshot>'
        )
        try:
