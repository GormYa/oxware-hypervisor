"""
OXware SIEM Exporter — CEF/LEEF/Syslog formatlarında log akışı
──────────────────────────────────────────────────────────────
...
_CFG  = Path("/var/lib/oxware/siem_config.json")
...
_DEFAULT = {
    ...
    "http_url":     "",
    "http_token":   "",
    "file_path":    "/var/log/oxware/siem.log",
    "vendor":       "OXware",
    "product":      "Hypervisor",
    "version":      "2.5.3",
}
...
def _format_syslog(event_type: str, severity: str, fields: dict, cfg: dict) -> str:
    ...
    return f"<{pri}>1 {ts} {host} oxware - {event_type} [{sd}]"
...
def test_connection() -> dict:
    ...
    try:
        emit("test_event", "info", {"src": "oxware-test", "msg": "SIEM connection test"})
