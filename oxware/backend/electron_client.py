"""
OXware Electron/Desktop Client — API tokens + manifest
────────────────────────────────────────────────────────
...
_CLIENTS_FILE = Path("/var/lib/oxware/desktop_clients.json")
...
_DOWNLOAD_BASE = "https://shinnasukha.github.io/OXware/releases"
_RELEASES = {
    "latest": "v0.1.0",
    "assets": [
        {
            ...
            "arch":     "x64",
            "format":   "AppImage",
            "url":      f"{_DOWNLOAD_BASE}/oxware-desktop-0.1.0-linux-x64.AppImage",
            "sha256":   None,
        },
        {
            "platform": "windows",
            "arch":     "x64",
            "format":   "exe",
            "url":      f"{_DOWNLOAD_BASE}/oxware-desktop-0.1.0-win-x64.exe",
            "sha256":   None,
        },
        {
            "platform": "macos",
            "arch":     "arm64",
            "format":   "dmg",
            "url":      f"{_DOWNLOAD_BASE}/oxware-desktop-0.1.0-macos-arm64.dmg",
            "sha256":   None,
        },
        {
            "platform": "macos",
            "arch":     "x64",
            "format":   "dmg",
            "url":      f"{_DOWNLOAD_BASE}/oxware-desktop-0.1.0-macos-x64.dmg",
            "sha256":   None,
        },
    ],
}
...
def _generate_token(client_id: str) -> str:
    ...
    raw   = f"oxware-desktop:{client_id}:{stamp}:{rand}"
...
def generate_client_config(server_url: str) -> dict:
    ...
    its connection to this OXware server.
    """
    if not server_url:
        raise ValueError("server_url required")
    return {
        "oxware_api": server_url.rstrip("/"),
        "api_version": "v1",
        "specversion": "2.5.12",
