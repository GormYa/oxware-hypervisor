"""
OXware Managed Cluster Federation
──────────────────────────────────
Federate multiple OXware nodes into a single control plane.

State:
  /etc/oxware/federation.json    member roster (id, url, token, region, role)
  /var/lib/oxware/federation_cache.json   last-seen status per member

A federation member is another OXware controller reachable over HTTPS.
...
def _request(member: dict, path: str, method: str = "GET",
    ...
    headers = {"Authorization": f"Bearer {member['token']}",
               ...
               "User-Agent": "OXware-Federation/1.0"}
