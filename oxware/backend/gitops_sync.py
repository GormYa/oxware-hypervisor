"""
gitops_sync.py — GitOps Sync (ArgoCD / Flux) for OXware
...
Config persisted to /var/lib/oxware/gitops.json
...
_CONFIG_FILE  = Path("/var/lib/oxware/gitops.json")
_SYNC_DIR     = Path("/var/lib/oxware/gitops_repo")
...
def _build_git_env(ssh_key: Optional[str]) -> dict:
    ...
    env = os.environ.copy()
    if ssh_key:
        key_path = Path("/tmp/oxware_gitops_key")
        try:
...
def generate_app_manifest() -> dict:
    ...
    provider = data.get("provider", "argocd")
    repo_url = data.get("repo_url", "https://github.com/example/oxware-gitops")
    branch   = data.get("branch", "main")
    path     = data.get("path", ".")
