...
OXware used `paramiko.AutoAddPolicy()` for every outbound SSH connection,
...
`/var/lib/oxware/known_hosts` plus a pending-prompts queue that the
panel surfaces so the operator can approve a new host fingerprint once.

Usage from the rest of the backend:

    from ssh_known_hosts import OxwarePolicy, load_known_hosts

    client.load_host_keys(load_known_hosts())
    client.set_missing_host_key_policy(OxwarePolicy())
...
log = logging.getLogger("oxware.ssh_known_hosts")

_KNOWN_HOSTS_PATH = Path("/var/lib/oxware/known_hosts")
_PENDING_PATH = Path("/var/lib/oxware/known_hosts_pending.json")
...
class OxwarePolicy:
    """A paramiko MissingHostKeyPolicy that:
      ...
      * trust-on-first-use is opt-in via OXWARE_SSH_TOFU=1 for migration
    """
    ...
    def missing_host_key(self, client, hostname, key):
        ...
        log.warning("ssh_known_hosts: queued first-contact prompt for "
                    "%s (%s)", hostname, fp)
        if os.environ.get("OXWARE_SSH_TOFU") == "1":
