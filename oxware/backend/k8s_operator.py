"""
k8s_operator.py — Kubernetes Operator for OXware VMs
OXware v2.5.10 Cloud/K8s

Features:
  - generate_crd() — OxwareVM CRD YAML (CustomResourceDefinition)
  - generate_operator_manifests() — Operator Deployment + RBAC YAML
  - list_managed_vms() — list OxwareVM CRs from cluster
  - reconcile_status() — summarize operator reconcile loop state

Config persisted to /var/lib/oxware/k8s_operator.json
...
_STATE_FILE    = Path("/var/lib/oxware/k8s_operator.json")
_lock          = threading.Lock()

_OPERATOR_IMAGE  = "ghcr.io/oxware/vm-operator:v2.5.10"
_CRD_GROUP       = "oxware.io"
_CRD_VERSION     = "v1alpha1"
_CRD_KIND        = "OxwareVM"
_CRD_PLURAL      = "oxwarevms"
_CRD_SINGULAR    = "oxwarevm"
_OPERATOR_NS     = "oxware-system"
...
def generate_crd() -> str:
    """Generate OxwareVM CustomResourceDefinition YAML."""
    crd_yaml = f"""\
          ...
          properties:
            spec:
              ...
              properties:
                name:
                  type: string
                  description: "VM name on OXware hypervisor"
                vcpus:
                  type: integer
                  minimum: 1
                  maximum: 256
                ...
                oxwareApiUrl:
                  type: string
                  description: "OXware API base URL (overrides operator default)"
                autoStart:
                  type: boolean
                  default: true
...
metadata:
  name: {_OPERATOR_NS}
  labels:
    app.kubernetes.io/managed-by: oxware
"""
...
kind: ServiceAccount
metadata:
  name: oxware-vm-operator
  namespace: {_OPERATOR_NS}
---
apiVersion: rbac.authorization.k8s.io/v1
kind: ClusterRole
metadata:
  name: oxware-vm-operator-role
rules:
...
metadata:
  name: oxware-vm-operator-binding
subjects:
  - kind: ServiceAccount
    name: oxware-vm-operator
    namespace: {_OPERATOR_NS}
  ...
  template:
    metadata:
      labels:
        app: oxware-vm-operator
    spec:
      serviceAccountName: oxware-vm-operator
      securityContext:
        runAsNonRoot: true
        runAsUser: 65532
      containers:
        - name: operator
          ...
          env:
            - name: OXWARE_API_URL
              valueFrom:
                configMapKeyRef:
                  name: oxware-operator-config
                  key: api_url
            - name: OXWARE_API_TOKEN
              valueFrom:
                secretKeyRef:
                  name: oxware-operator-secret
                  key: api-token
...
metadata:
  ...
  namespace: {_OPERATOR_NS}
data:
  api_url: "http://oxware-api.{_OPERATOR_NS}.svc.cluster.local:5000"
...
def list_managed_vms() -> list:
    """List OxwareVM custom resources from the cluster."""
    rc, out, _ = _kubectl(
...
def reconcile_status() -> dict:
    ...
    rc, out, _ = _kubectl(
        ["get", "deployment", "oxware-vm-operator", "-n", _OPERATOR_NS, "-o", "json"]
    )
