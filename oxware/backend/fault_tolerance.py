"""
OXware Fault Tolerance Manager
━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Checkpoint-based VM protection:
  - create_ft_pair(primary_vm_id, secondary_pool) → creates secondary VM from checkpoint
  - get_ft_status(vm_id) → replication lag, last checkpoint time, status
  - trigger_failover(vm_id) → promote secondary, update IPAM
  - sync_checkpoint(vm_id) → manual checkpoint + delta sync
  - remove_ft(vm_id) → remove pairing

Uses: libvirt checkpoint API + qemu-img convert for delta sync.
State: /var/lib/oxware/ft_pairs.json
"""
...
FT_STATE_PATH = Path("/var/lib/oxware/ft_pairs.json")
...
def _create_checkpoint(dom, checkpoint_name: str) -> None:
  ...
  <description>OXware FT checkpoint</description>
</domaincheckpoint>"""
