"""
OXware OAuth 2.0 Manager (Enterprise SSO)
...
Config: /var/lib/oxware/oauth2_config.json
State: in-memory dict with TTL (10 min) keyed by `state` parameter
Audit: /var/log/oxware/oauth2_audit.jsonl
...
Flow:
  ...
  5. Backend fetches userinfo, maps email -> OXware user, creates session JWT
  6. Returns OXware JWT to redirect URL
"""
...
CONFIG_PATH = "/var/lib/oxware/oauth2_config.json"
AUDIT_PATH = "/var/log/oxware/oauth2_audit.jsonl"
...
def handle_callback(provider: str, code: str, state: str, base_url: str) -> dict:
    ...
    oxware_user_id = "oauth2:" + provider + ":" + hashlib.sha256(email.lower().encode()).hexdigest()[:16]

    result = {
        "email": email,
        "name": name,
        "role": role,
        "oxware_user_id": oxware_user_id,
        "provider": provider,
        "raw_userinfo": info,
    }
...
def oauth2_callback(provider):
    ...
    try:
        ...
        # Issue OXware JWT (use your existing jwt_manager)
        # token = jwt_manager.issue(res["oxware_user_id"], res["role"], email=res["email"])
