#!/usr/bin/env python3
"""
OXware Usage Telemetry — Encrypted IP/Usage Collector
------------------------------------------------------
Hangi IP'lerin sisteme login olduğunu şifreli şekilde kaydeder.
Opsiyonel: şifreli veriyi özel bir GitHub Gist'e gönderir.

Kurulum:
  - Bu modülü oxware/backend/ içine kopyalayın
  - app.py'de import edin
  - collect_login(ip, username) ile login'lerde çağırın

Sadece SIZE özel: şifreleme anahtarı TELEMETRY_KEY ortam değişkeninde
ya da /etc/oxware/.telemetry_key dosyasında tutulur.
"""
...
# ── Konfigürasyon ─────────────────────────────────────────────────────────────
_STORE_PATH   = Path("/var/lib/oxware/telemetry.enc")   # şifreli veri dosyası
_KEY_FILE     = Path("/etc/oxware/.telemetry_key")       # AES anahtarı
_GIST_ID_FILE = Path("/etc/oxware/.telemetry_gist")      # GitHub Gist ID
_GH_TOKEN_FILE= Path("/etc/oxware/.telemetry_token")     # GitHub token
_LOCK         = threading.Lock()

# ── Anahtar yönetimi ──────────────────────────────────────────────────────────

def _get_key() -> bytes:
    """32-byte AES anahtarı — dosyadan oku, yoksa üret."""
    ...
    # İlk çalışma: rastgele anahtar üret
    import secrets
...
# ── Basit XOR-CTR şifreleme (no external libs needed) ─────────────────────────

def _keystream(key: bytes, nonce: bytes, length: int) -> bytes:
    """AES yetersiz ortamlar için SHA-256 tabanlı CTR keystream."""
    ks = bytearray()
    counter = 0
    while len(ks) < length:
...
def _decrypt(blob: bytes, key: bytes) -> bytes:
    """Deşifrele + MAC doğrula. Hata: ValueError."""
    if len(blob) < 25:
    ...
    if not hmac.compare_digest(mac, exp):
        raise ValueError("HMAC mismatch — key yanlış ya da veri bozuk")
    ...
    def _write():
        with _LOCK:
            try:
                ...
                pass  # telemetry hatası asla ana işlemi durdurmamalı
    threading.Thread(target=_write, daemon=True).start()


def get_stats() -> dict:
    """Şifresi çözülmüş istatistikleri döndür (sadece admin için)."""
    with _LOCK:
        try:
            key     = _get_key()
            records = _load_records(key)
        except Exception:
            return {"error": "Veri okunamadı — anahtar yanlış olabilir"}
...
def push_to_gist() -> dict:
    """
    Şifreli blobu GitHub Gist'e gönder.
    /etc/oxware/.telemetry_gist  → Gist ID (boşsa yeni gist oluşturur)
    /etc/oxware/.telemetry_token → GitHub Personal Access Token (gist scope)
    """
    try:
    ...
    if not token:
        return {"ok": False, "error": "GitHub token yok — /etc/oxware/.telemetry_token dosyasına yazın"}
...
# ── CLI: python3 telemetry/collector.py komutu ────────────────────────────────
if __name__ == "__main__":
    ...
    if cmd == "stats":
        ...
        if "error" in stats:
            ...
            print(f"  Son 7 gün:         {stats['unique_ips_7d']} IP, {stats['logins_7d']} login")
            print(f"  Son 30 gün:        {stats['unique_ips_30d']} IP, {stats['logins_30d']} login")
            print(f"\nTüm IP'ler ({stats['unique_ips']}):")
        ...
        result = push_to_gist()
        if result["ok"]:
            print(f"GitHub Gist güncellendi: {result.get('url','')}")
        else:
            print(f"Push hatası: {result.get('error','')}")
        ...
        print(f"Kullanım: python3 collector.py [stats|push|key]")
