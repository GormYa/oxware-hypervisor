...
Open an issue at https://github.com/ShinnAsukha/oxware-hypervisor/issues.

Before opening, please:

1. Search existing issues to avoid duplicates.
2. Run `sudo /opt/oxware/repair.sh --diagnose` and attach the output. The
...
3. Include exact steps to reproduce, the expected result, and the actual
   result.
4. Include OS, kernel version, and OXware version (`oxware --version`).
...
```
git clone https://github.com/ShinnAsukha/oxware-hypervisor.git
cd oxware-hypervisor
python3 -m venv .venv
...
```
export OXWARE_CONFIG=./dev-config.yaml
...
OXware is released under the MIT License. By submitting a pull request you
