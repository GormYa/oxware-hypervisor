...
if [[ $MODE_RESTORE_NETWORK -eq 1 ]] || [[ "${1:-}" == "--restore-network" ]]; then
 ...
 timeout 30 netplan try --timeout 120 </dev/null && log "Ağ geri yüklendi OK" || netplan apply
...
if [[ $MODE_REMOVE_HARDENING -eq 1 ]]; then
 ...
 sleep 3
 systemctl is-active --quiet oxware && log "oxware.service başlatıldı OK" || \
...
RESET
 ...
 log "Kimlik bilgileri güncellendi OK"
...
NETPLANCFG
 chmod 600 "$NP"
 timeout 30 netplan try --timeout 120 </dev/null && log "oxbr0 oluşturuldu OK" || \
...
if systemctl is-active --quiet oxware; then
 echo -e "${GREEN}╔══════════════════════════════════════════════════════╗"
 echo -e "║ OXware Onarım Tamamlandı! OK ║"
 echo -e "╠══════════════════════════════════════════════════════╣"
 echo -e "║${NC} ${GREEN}║"
