# -*- coding: utf-8 -*-
"""
OXware API helper for Ansible modules.
...
class OxwareClient(object):
        ...
        for attempt in range(retries + 1):
            try:
                ...
                try:
                    ...
                    j = {"error": body}
                raise RuntimeError("OXware API %d: %s" % (e.code, j.get("error", body)))
            except URLError as e:
                last_err = e
                if attempt < retries:
                    time.sleep(1.5 * (attempt + 1))
                    continue
                raise RuntimeError("OXware API connect failed: %s" % str(e))
        raise RuntimeError("OXware API unreachable: %s" % str(last_err))
    ...
    def login(self):
        ...
        if not self.token:
            raise RuntimeError("OXware login failed: token missing in response")
        return self.token
...
def build_client(module):
    """Create OxwareClient from module params."""
    p = module.params
