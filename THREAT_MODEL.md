...
Threat model v1.0 - applies to OXware 2.6.x.

This document describes the security threat model for the OXware Hypervisor management
stack using the STRIDE methodology. It is written for operators deploying OXware in a
...
- A single OXware host running Ubuntu 22.04 LTS or Debian 12.
...
- The local audit log at `/var/lib/oxware/audit.log`.
- Local state directory `/var/lib/oxware/` and configuration in `/etc/oxware/`.
...
- The Flask process runs as an unprivileged user (`oxware`) and uses `sudo` with a
  narrow allowlist for libvirt and disk operations.
- Operators rotate JWT secrets at install time and store them in
  `/etc/oxware/oxware.conf` with mode 0600.
...
[ nginx ]  <-- trust boundary 1: external network to host
     |  HTTP loopback
     v
[ Flask process (user: oxware) ]  <-- trust boundary 2: web layer to backend
     |  Unix socket / sudo
     v
...
| API keys for cloud providers | `/etc/oxware/oxware.conf` | High |
| JWT signing secret | `/etc/oxware/oxware.conf` | Critical |
| Audit log | `/var/lib/oxware/audit.log` | High - integrity |
| Session cookies | Browser, server memory | Medium |
| User password hashes | `/var/lib/oxware/users.json` | High |
| OAuth client secrets | `/etc/oxware/oxware.conf` | High |
| Backup encryption keys | `/etc/oxware/backup.key` | Critical |
...
| I-6 | Console screenshots in recording directory readable by other users | Recording directory mode 0700, owned by `oxware` user (SEC-006) |
...
| ID | Threat | Mitigation |
|----|--------|------------|
| E-1 | Privilege escalation via setuid binaries | The OXware install does not add any setuid binaries; `sudo` rules are restricted to a fixed allowlist in `/etc/sudoers.d/oxware` |
...
| Version | Date | Notes |
|---------|------|-------|
| 1.0 | 2026-06-06 | Initial threat model, covers OXware 2.6.x |
