#!/bin/bash
# OXware Host Bridge Setup — opt-in, SSH-safe (netplan try ile 120s rollback)
# Kullanım: sudo /opt/oxware/scripts/setup-bridge.sh
set -uo pipefail
...
cat > /etc/netplan/60-oxware-bridge.yaml << NP
network:
  version: 2
  ethernets:
    ${PIFACE}:
  ...
  bridges:
    oxbr0:
      ...
      parameters:
        ...
        forward-delay: 0
NP
chmod 600 /etc/netplan/60-oxware-bridge.yaml
