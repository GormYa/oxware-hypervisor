...
# ============================================================================
# OXware OS Rebrander
# Converts Ubuntu/Debian identity into "OXware Hypervisor" identity.
# Idempotent — safe to re-run. Backs up originals to /etc/oxware/.os-backup/
...
BACKUP_DIR="/etc/oxware/.os-backup"
BRAND_NAME="OXware Hypervisor"
BRAND_ID="oxware"
BRAND_ID_LIKE="debian"
BRAND_HOME_URL="https://github.com/ShinnAsukha/oxware-hypervisor"
BRAND_SUPPORT_URL="https://github.com/ShinnAsukha/oxware-hypervisor/issues"
BRAND_BUG_URL="https://github.com/ShinnAsukha/oxware-hypervisor/issues"

# ---------------------------------------------------------------------------
# Detect OXware version
# ---------------------------------------------------------------------------
detect_version() {
    local v=""
    if [ -f "/opt/oxware/oxware/backend/app.py" ]; then
        v="$(grep -m1 -oE 'v[0-9]+\.[0-9]+\.[0-9]+' /opt/oxware/oxware/backend/app.py 2>/dev/null | head -n1 | sed 's/^v//')"
    fi
...
OXWARE_VERSION="$(detect_version)"
info "OXware version detected: ${C_BOLD}${OXWARE_VERSION}${C_RESET}"
...
# ---------------------------------------------------------------------------
is_rebranded() {
    [ -f /etc/os-release ] && grep -q "OXware" /etc/os-release
}
...
DISTRIB_ID=OXware
DISTRIB_RELEASE=${OXWARE_VERSION}
DISTRIB_CODENAME=hypervisor
DISTRIB_DESCRIPTION="${BRAND_NAME} ${OXWARE_VERSION}"
EOF
...
# ---------------------------------------------------------------------------
rebrand_issue() {
    step "Installing OXware login banner (/etc/issue, /etc/issue.net)"
    backup_file /etc/issue
    backup_file /etc/issue.net

    local src="$SCRIPT_DIR/oxware-issue"
    if [ ! -f "$src" ]; then
        warn "Template $src not found — writing minimal banner."
        cat > /etc/issue <<EOF
${BRAND_NAME} ${OXWARE_VERSION}
Kernel \\r on an \\m

EOF
    else
        # Substitute version placeholder if needed, then install
        sed "s/2\.6\.1/${OXWARE_VERSION}/g" "$src" > /etc/issue
    fi
    chmod 644 /etc/issue
...
rebrand_hostname() {
    step "Hostname check"
    if [ "${OXWARE_REBRAND_HOSTNAME:-0}" != "1" ]; then
        info "OXWARE_REBRAND_HOSTNAME!=1, skipping hostname change."
        return 0
    fi
    local current
    ...
    case "$current" in
        localhost|ubuntu|debian|"")
            ...
            echo "oxware" > /etc/hostname
            hostnamectl set-hostname "oxware" 2>/dev/null || true
            log "Hostname set to 'oxware' (was '$current')."
            ;;
        *)
...
rebrand_grub() {
    ...
    if grep -q '^GRUB_DISTRIBUTOR=' /etc/default/grub; then
        sed -i 's|^GRUB_DISTRIBUTOR=.*|GRUB_DISTRIBUTOR="OXware"|' /etc/default/grub
    else
        echo 'GRUB_DISTRIBUTOR="OXware"' >> /etc/default/grub
    fi
    log "GRUB_DISTRIBUTOR=OXware set."
...
install_plymouth_theme() {
    ...
    local src="/opt/oxware/assets/plymouth"
    if [ ! -d "$src" ]; then
    ...
    local dst="/usr/share/plymouth/themes/oxware"
    mkdir -p "$dst"
    cp -a "$src/." "$dst/"
    plymouth-set-default-theme -R oxware >/dev/null 2>&1 \
        && log "Plymouth theme 'oxware' set." \
...
# ---------------------------------------------------------------------------
ensure_motd_header() {
    step "Ensuring /etc/update-motd.d/00-header is OXware-branded"
    local dir="/etc/update-motd.d"
    [ -d "$dir" ] || { info "$dir not present, skipping."; return 0; }
    local hdr="$dir/00-header"

    if [ -f "$hdr" ] && grep -q "OXware" "$hdr"; then
        info "00-header already OXware-branded."
        return 0
    fi
    backup_file "$hdr"
    cat > "$hdr" <<EOF
#!/bin/sh
printf "\n"
printf "  OXware Hypervisor ${OXWARE_VERSION}  |  \$(uname -r)\n"
...
# ---------------------------------------------------------------------------
# Step 11: /usr/local/bin/oxware-version
# ---------------------------------------------------------------------------
install_version_helper() {
    step "Installing /usr/local/bin/oxware-version"
    cat > /usr/local/bin/oxware-version <<EOF
#!/bin/sh
# OXware version helper — prints full identity string.
. /etc/os-release 2>/dev/null || true
KERNEL="\$(uname -r)"
ARCH="\$(uname -m)"
HOST="\$(hostname)"
cat <<INFO
OXware Hypervisor ${OXWARE_VERSION}
  Kernel:    \$KERNEL (\$ARCH)
  Hostname:  \$HOST
  Build ID:  \${OXWARE_BUILD:-${OXWARE_VERSION}}
  Web UI:    https://\$(hostname -I 2>/dev/null | awk '{print \$1}'):8006
INFO
EOF
    chmod 755 /usr/local/bin/oxware-version
    log "/usr/local/bin/oxware-version installed."
}

# ---------------------------------------------------------------------------
# Step 12: /etc/profile.d/oxware.sh
# ---------------------------------------------------------------------------
install_profile_d() {
    step "Installing /etc/profile.d/oxware.sh"
    local src="$SCRIPT_DIR/oxware.sh"
    local dst="/etc/profile.d/oxware.sh"
    if [ -f "$src" ]; then
...
install_uname_wrapper() {
    ...
    if [ "${OXWARE_BRAND_UNAME:-0}" != "1" ]; then
        info "OXWARE_BRAND_UNAME!=1, skipping uname wrapper."
        return 0
    fi
    cat > /usr/local/bin/uname-wrapper <<EOF
#!/bin/sh
# OXware-branded uname wrapper.
REAL="\$(/bin/uname "\$@")"
case " \$* " in
    *" -a "*|*" --all "*)
        echo "\${REAL} OXware-Hypervisor/${OXWARE_VERSION}"
        ;;
    *)
        echo "\$REAL"
        ;;
...
do_restore() {
    ...
    rm -f /etc/profile.d/oxware.sh
    rm -f /usr/local/bin/oxware-version
...
main() {
    ...
    if is_rebranded; then
        info "System already shows OXware identity — re-running idempotently."
    fi
    ...
    echo
    log "${C_BOLD}OXware OS rebrand complete.${C_RESET}"
    info "Run 'oxware-version' to verify."
