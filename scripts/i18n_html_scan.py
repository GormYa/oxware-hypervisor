...
letters (ğ/ş/ı/İ/ç/ö/ü/Ğ/Ş/Ç/Ö/Ü) or a known TR word is a candidate. We then
...
TR_CHARS = set("çğıİöşüÇĞIÖŞÜ")
TR_HINT_WORDS = {
    "ve", "veya", "ile", "için", "olarak", "bir", "bu", "şu", "ben",
    "yeni", "ekle", "sil", "kaydet", "iptal", "kapat", "düzenle",
    "oluştur", "yükle", "indir", "yenile", "ara", "ayar", "ayarlar",
    "kullanıcı", "kullanıcılar", "şifre", "yönetici", "rol", "grup",
    "ağ", "ağı", "ağa", "ağdan", "depolama", "yedek", "yedekleme",
    "izleme", "uyarı", "olay", "günlük", "geçmiş", "rapor", "yedek",
    "sistem", "donanım", "yazılım", "bilgi", "durum", "boş", "dolu",
    "açık", "kapalı", "evet", "hayır", "tamam", "vazgeç", "geri",
    "ileri", "önceki", "sonraki", "varsayılan", "özel", "genel",
    "sanal", "makine", "makineler", "anlık", "görüntü",
}
...
def looks_turkish(s: str) -> bool:
    ...
    if len(s) < 2:
        return False
    # Pure ASCII numeric / symbol → skip
    if not any(c.isalpha() for c in s):
        return False
    # Has TR-specific letters → Turkish
    if any(c in TR_CHARS for c in s):
        return True
    # Has TR hint words (lowercased token match)
    tokens = re.findall(r"[A-Za-zçğıİöşüÇĞIÖŞÜ]+", s.lower())
...
def text_nodes(html_body: str):
    # Strip <script>, <style>, <noscript>, <template>, <svg> contents — we only
    ...
    # Now split on tags — what's outside tags is text content.
    out = []
    pos = 0
