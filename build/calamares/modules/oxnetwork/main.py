...
# -*- coding: utf-8 -*-
"""
OXware Ağ Yapılandırması — Calamares Python viewmodule
Proxmox VE tarzı: live sistem ağını Calamares içinde yapılandırır.
"""
...
class NetworkWidget(QWidget):
    ...
    def _setup_ui(self):
        ...
        self._e_host = make_row('Hostname:',    'oxware-node')
        self._e_host.setText(_hostname())
        self._e_ip   = make_row('IP Adresi:',   '192.168.1.100')
        self._e_mask = make_row('Ağ Maskesi:',  '255.255.255.0')
        self._e_mask.setText('255.255.255.0')
        self._e_gw   = make_row('Ağ Geçidi:',   '192.168.1.1')
    ...
    def _refresh_iface(self):
        ...
        if info['ip']:
            ...
            self._status_lbl.setText(f'{iface}: bağlantı yok')

    def apply(self):
        """Live sisteme ağ ayarlarını uygula — ViewStep.onLeave() tarafından çağrılır."""
        iface    = self._iface_combo.currentText()
        hostname = self._e_host.text().strip() or 'oxware-node'
        ...
        try:
            ...
            # Değerleri oxware_install için sakla
