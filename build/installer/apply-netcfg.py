#!/usr/bin/env python3
"""
OXware — apply-netcfg.py
Kurulumdan sonra ağ yapılandırmasını kurulu sisteme uygular.
Kullanım: apply-netcfg.py <config.json> [mount_point]
"""
...
INSTALL_CFG = "/tmp/oxware-install-config.json"


def find_root_mount():
    """Kurulu sistemin root partisyonunun mount noktasını bul."""
    ...
    # 3. oxware-install-config.json'dan disk bul, mount et
    if os.path.exists(INSTALL_CFG):
        try:
            ...
            print(f"[WARN] install config okunamadı: {e}")
...
def _mount_disk(disk):
    ...
    # Partition adayları: disk + 2 (EFI=1, root=2) veya disk + 1
...
def write_interfaces(mp, cfg):
    ...
    hostname = cfg.get("hostname",  "oxware-node")
    ...
    try:
        ...
        print(f"[WARN] /etc/hosts yazılamadı: {e}")

    # /etc/network/interfaces
    ifaces_path = f"{mp}/etc/network/interfaces"
    if mode == "dhcp":
        content = (
            "# OXware auto-generated\n"
    ...
    try:
        with open(ifaces_path, "w") as f:
            f.write(content)
        print(f"[OK] /etc/network/interfaces yazıldı ({mode})")
    except Exception as e:
        print(f"[WARN] interfaces yazılamadı: {e}")
    ...
    try:
        ...
        print(f"[WARN] resolv.conf yazılamadı: {e}")
...
def main():
    ...
    if not os.path.exists(CFG_PATH):
        print(f"[ERR] Config bulunamadı: {CFG_PATH}")
        sys.exit(1)
    ...
    mp = find_root_mount()
    if not mp:
        print("[ERR] Kurulu sistem mount edilemedi — ağ ayarı uygulanamadı")
        sys.exit(1)

    print(f"[*] Mount point: {mp}")
    write_interfaces(mp, cfg)

    # Geçici mount ise unmount et
    if mp == "/mnt/oxinstalled":
        subprocess.run(["umount", mp], capture_output=True)
        print("[OK] Unmount edildi")

    print("[OK] Ağ yapılandırması tamamlandı")
